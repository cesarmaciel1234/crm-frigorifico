...
def sangria_diaria() -> dict[str, Any]:
    ...
    for row in rows:
        r = dict(row)
        # Ignorar si ya está pagada por completo
        if r["cuotas"] is not None and r["cuotas_pagadas"] >= r["cuotas"]:
            continue
        total_intereses += max(0.0, r["pagar"] - r["recibido"])
        ops_activas += 1
        
    sangria = total_intereses / 30.0
    return {
        "intereses_totales": round(total_intereses, 2),
        "sangria_diaria": round(sangria, 2),
        "sangria_financiera_diaria": round(sangria, 2),
        "sangria_cheques_diaria": 0.0,
        "saldo_cheques_pendiente": 0.0,
        "cheques_activos": 0,
...
def panel_activo() -> dict[str, Any]:
    with get_db() as conn:
        rem = conn.execute(
            """
            SELECT
                COALESCE(SUM(costo_total_logistica), 0) AS costo,
                COALESCE(SUM(precio_venta_total - costo_total_logistica), 0) AS ganancia,
    ...
    activo_costo = float(rem["costo"])
    ganancia = float(rem["ganancia"])
...
def proyeccion_liberacion(excedente_mensual_manual: Optional[float] = None) -> dict[str, Any]:
    with get_db() as conn:
        ...
        rem_row = conn.execute(
            """
            SELECT
                COALESCE(SUM(precio_venta_total - costo_total_logistica), 0) AS margen,
