...
def marcar_cliente_incobrable(cliente_id: int) -> int:
    with get_db() as conn:
        ...
        # 3. Reiniciar saldo_actual, bloquear techo_deuda = 0 y asignar scoring 'C'
        conn.execute(
            "UPDATE clientes SET saldo_actual = 0, techo_deuda = 0, scoring = 'C' WHERE id = ?",
            (cliente_id,)
        )
...
def list_perdidas_acumuladas() -> list[dict]:
    ...
    out = []
    tasa_anual = 0.15  # 15% costo de oportunidad anual
